...
<div class="flex items-center justify-center min-h-screen ">
    <div class="w-full p-8 bg-gray-500 rounded shadow-md sm:w-96">
        <h1 class="mb-4 text-4xl font-semibold text-center text-white">Update Supplier</h1>
        <form action="{{ route('suppliers.update',$supplier) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="supplier" name="supplier" class="w-full p-2 border rounded"
                    placeholder="enter a supplier name" value="{{ old('supplier',$supplier->supplier) }}">
                @error('supplier')
            ...
            <div class="mb-4">
                ...
                <input type="text" id="address" name="address" class="w-full p-2 border rounded"
                    placeholder="write the address" value="{{ old('adress',$supplier->address) }}">
                @error('address')
                <p class="p-2 my-2 text-sm text-center text-white bg-red-500 rounded-lg">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <input type="number" id="phone" name="phone" class="w-full p-2 border rounded"
                    placeholder="write your phone number" value="{{ old('phone',$supplier->phone) }}">
                @error('phone')
                <p class="p-2 my-2 text-sm text-center text-white bg-red-500 rounded-lg">{{ $message }}</p>
                @enderror
            </div>
